...
class ReplicaDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.rgb_dir = os.path.join(self.root_dir,"results")
        self.depth_dir = os.path.join(self.root_dir,"results")

        # intrinsic info
        self.Ks = np.loadtxt(os.path.join(root_dir, K_file)).reshape(3, 3)
    ...
    def __getitem__(self, idx):
        # read raw data from path -------------------------------------------
        rgb_file = os.path.join(self.rgb_dir, "frame%06d.png"%(idx))
        ndepth_file = os.path.join(self.depth_dir, "ndepth%06d.png"%(idx))
        depth_file = os.path.join(self.depth_dir, "depth%06d.png"%(idx))
        
        rgb = cv2.imread(rgb_file)
        ndepth = cv2.imread(ndepth_file, -1) 
        depth = cv2.imread(depth_file, -1) 
        T = self.Ts[idx] 

        sample = {"rgb": rgb, "depth": depth, "T": T, "ndepth": ndepth}
