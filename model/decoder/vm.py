...
class VMDecoding(nn.Module):
    ...
    def forward(self, in_tensor: TensorType["bs":..., "input_dim"], do_grad=False) -> TensorType["bs":..., "output_dim"]:
        ...
        # if do_grad:
        #     line_coef_grad = torch.zeros_like(self.line_coef)
        ...
        #     dsdx = line_feature_grad[0] * line_features[[1,2]].prod(dim=0)
        ...
        #     feature_grad = torch.cat([dsdx, dsdy, dsdz], dim=-1)
        #     feature_grad = torch.moveaxis(feature_grad.view(self.num_components, *in_tensor.shape), 0, -1)
        # else:
        #     feature_grad = None
